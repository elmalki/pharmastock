...
const fetchCommandes = (start,end)=>{
    axios.post('/api/peremption',{start:start,end:end})
        .then(response=>{
            response.data.forEach(item=>{
                if(!calendarApp.events.get(item.id))
                    calendarApp.events.add({
                        ...
                        description:`La quantité restante: ${item.qte}`
                    })
            })
        })
}
...
<template>
    <AppLayout title="Calendrier de péremptions">
        <div class="mx-auto max-w-7xl h-1/4 my-10" >
            <Breadcrumbs class="mb-4" :pages="[{name:'Produits',href:route('produits.index'),current:false},{name:'Date Péremption',href:route('produits.peremption'),current:true}]"></Breadcrumbs>
